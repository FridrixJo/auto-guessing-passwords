...
class UsersDB:
    ...
    def set_status(self, user_id, status):
        try:
            self.sql.execute("UPDATE `users` SET status = ? WHERE user_id = ?", (status, user_id))
        except Exception as e:
            print(e, "set_status")
        return self.db.commit()
    ...
    def get_status(self, user_id):
        try:
            result = self.sql.execute("SELECT status FROM users WHERE user_id = ?", (user_id,))
            return result.fetchall()[0][0]
        except Exception as e:
            print(e, "get_status")
    ...
    def set_count(self, user_id, count):
        try:
            self.sql.execute("UPDATE `users` SET count = ? WHERE user_id = ?", (count, user_id))
        except Exception as e:
            print(e, "set_count")
        return self.db.commit()
    ...
    def get_count(self, user_id):
        try:
            result = self.sql.execute("SELECT count FROM users WHERE user_id = ?", (user_id,))
            return result.fetchall()[0][0]
        except Exception as e:
            print(e, "get_count")
    ...
    def increment_count(self, user_id):
        try:
            self.sql.execute("UPDATE `users` SET count = ? WHERE user_id = ?", (self.get_count(user_id=user_id) + 1, user_id))
        except Exception as e:
            print(e, "set_count")
        return self.db.commit()
